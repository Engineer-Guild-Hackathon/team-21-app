...
class EmotionAnalyzer:
    ...
    def analyze_emotion(
        ...
        with torch.no_grad():
            ...
            # 行動データの特徴抽出
            behavioral_features = (
                self._extract_behavioral_features(behavioral_data)
                if behavioral_data
                else None
            )
            ...
            # 確信度の計算
            confidence = torch.softmax(emotion_scores, dim=0)[
                primary_emotion_idx
            ].item()
    ...
    def _extract_behavioral_features(
        self, behavioral_data: Dict[str, float]
    ) -> torch.Tensor:
        """行動データから特徴を抽出"""
        # 行動特徴の正規化と変換
        features = [
...
class EmotionClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.text_processor = nn.Sequential(
            nn.Linear(bert_hidden_size, hidden_size), nn.ReLU(), nn.Dropout(0.2)
        )

        # 行動特徴の処理層
        self.behavioral_processor = nn.Sequential(
            nn.Linear(behavioral_feature_size, hidden_size), nn.ReLU(), nn.Dropout(0.2)
        )
    ...
    def forward(
        ...
        if text_features is None:
            text_features = torch.zeros((batch_size, hidden_size)).to(
        ...
        if behavioral_features is None:
            behavioral_features = torch.zeros((batch_size, hidden_size)).to(
                next(self.parameters()).device
            )
        else:
        ...
        attended_features, _ = self.attention(
            text_features, behavioral_features, behavioral_features
        )

        # 特徴の結合
        combined_features = torch.cat(
            [attended_features.squeeze(0), behavioral_features.squeeze(0)], dim=-1
        )
...
class EmotionTracker:
    ...
    def get_emotion_trend(self) -> Dict[str, float]:
        ...
        return {
            emotion.value: np.mean(scores) for emotion, scores in emotion_scores.items()
        }
