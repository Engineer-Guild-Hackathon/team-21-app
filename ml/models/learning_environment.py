...
class NonCogLearningEnvironment(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Dict(
            {
                # 非認知能力スコア（0-1の範囲）
                "perseverance": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # やり抜く力
                "self_control": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # 自制心
                "cooperation": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # 協調性
                "curiosity": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # 好奇心
                # 学習進捗
                "current_level": spaces.Discrete(10),  # 現在のレベル
                "challenge_count": spaces.Discrete(100),  # チャレンジ回数
                "success_rate": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),  # 成功率
                # 感情状態（-1: ネガティブ, 0: 中立, 1: ポジティブ）
                "emotional_state": spaces.Box(
        ...
        self.action_space = spaces.Dict(
            {
                # タスクの種類
                "task_type": spaces.Discrete(
                    4
                ),  # [問題解決, 協力課題, 創造的課題, 自己管理課題]
                # タスクの難易度
                "difficulty": spaces.Discrete(5),  # [1-5の難易度]
                # フィードバックスタイル
                "feedback_style": spaces.Discrete(
                    3
                ),  # [励まし重視, 解説重視, バランス型]
            }
        )
    ...
    def _calculate_emotional_change(
        self, success: bool, action: Dict[str, int]
    ) -> float:
        """感情状態の変化量を計算"""
    ...
    def _update_state(self, action: Dict[str, int], task_outcome: Dict[str, Any]):
        ...
        for ability, gain in ability_gains.items():
            ...
            self.state[ability] = np.array(
                [min(1.0, current_value + gain)], dtype=np.float32
            )
        ...
        new_success_rate = (
            current_success_rate * (self.state["challenge_count"] - 1)
            + float(task_outcome["success"])
        ) / self.state["challenge_count"]
        self.state["success_rate"] = np.array([new_success_rate], dtype=np.float32)

    def _calculate_reward(
        self, action: Dict[str, int], task_outcome: Dict[str, Any]
    ) -> float:
        """報酬の計算"""
        # 基本報酬（タスクの成功/失敗）
