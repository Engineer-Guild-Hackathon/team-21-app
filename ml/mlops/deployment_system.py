...
class DeploymentSystem:
    ...
    def deploy_model(self, deployment_config: DeploymentConfig) -> bool:
        ...
        try:
            ...
            model, metadata = self.model_registry.load_model(
                deployment_config.model_name, deployment_config.version
            )
            ...
            if deployment_config.strategy == DeploymentStrategy.ROLLING:
                ...
                raise ValueError(
                    f"Unknown deployment strategy: {deployment_config.strategy}"
                )
    ...
    def _build_docker_image(self, model: nn.Module, metadata: Any) -> str:
        """Dockerイメージの作成"""
        try:
            # 一時ディレクトリの作成
            ...
            self.docker_client.images.build(path=str(build_dir), tag=tag, rm=True)
    ...
    def _rolling_deployment(self, image_tag: str, config: DeploymentConfig) -> bool:
        """ローリングデプロイメント"""
        try:
            # デプロイメント定義
            deployment = client.V1Deployment(
                metadata=client.V1ObjectMeta(name=f"{config.model_name}-deployment"),
                spec=client.V1DeploymentSpec(
                    ...
                    template=client.V1PodTemplateSpec(
                        metadata=client.V1ObjectMeta(labels={"app": config.model_name}),
            ...
            self.logger.error(
                "Rolling deployment failed", model_name=config.model_name, error=str(e)
            )
            return False

    def _blue_green_deployment(self, image_tag: str, config: DeploymentConfig) -> bool:
        """Blue-Greenデプロイメント"""
        try:
            # 現在のアクティブ環境の確認
            current_service = self.k8s_core.read_namespaced_service(
                name=f"{config.model_name}-service", namespace="default"
            )
            ...
            # 新環境のデプロイメント
            deployment = client.V1Deployment(
                metadata=client.V1ObjectMeta(name=f"{config.model_name}-{new_color}"),
                spec=client.V1DeploymentSpec(
                    replicas=config.scaling_config.get("replicas", 3),
                    selector=client.V1LabelSelector(
                        match_labels={"app": config.model_name, "color": new_color}
                    ),
                    template=client.V1PodTemplateSpec(
                        metadata=client.V1ObjectMeta(
                            labels={"app": config.model_name, "color": new_color}
                        ),
                        spec=client.V1PodSpec(
                            containers=[
                                client.V1Container(
            ...
            service = client.V1Service(
                metadata=client.V1ObjectMeta(name=f"{config.model_name}-service"),
                spec=client.V1ServiceSpec(
                    selector={"app": config.model_name, "color": new_color},
                    ports=[client.V1ServicePort(port=80, target_port=8000)],
                ),
            )

            self.k8s_core.replace_namespaced_service(
                name=f"{config.model_name}-service", namespace="default", body=service
            )
            ...
            if config.rollback_config.get("keep_old_deployment", False):
                ...
                self.k8s_apps.delete_namespaced_deployment(
                    name=f"{config.model_name}-{current_color}", namespace="default"
                )
    ...
    def _canary_deployment(self, image_tag: str, config: DeploymentConfig) -> bool:
        """Canaryデプロイメント"""
        try:
            # Canaryデプロイメントの作成
            canary_deployment = client.V1Deployment(
                metadata=client.V1ObjectMeta(name=f"{config.model_name}-canary"),
                spec=client.V1DeploymentSpec(
                    replicas=1,  # Canaryは小規模に開始
                    selector=client.V1LabelSelector(
                        match_labels={"app": config.model_name, "version": "canary"}
                    ),
                    template=client.V1PodTemplateSpec(
                        metadata=client.V1ObjectMeta(
                            labels={"app": config.model_name, "version": "canary"}
                        ),
                        spec=client.V1PodSpec(
                            containers=[
                                client.V1Container(
            ...
            # Canaryデプロイメントの作成
            self.k8s_apps.create_namespaced_deployment(
                namespace="default", body=canary_deployment
            )
            ...
            for percentage in [25, 50, 75, 100]:
                ...
                if not self._verify_canary_metrics():
                    self.logger.warning(
                        "Canary metrics check failed", percentage=percentage
                    )
                    return False

                # トラフィック比率の更新
                self._update_traffic_split(config.model_name, percentage)
            ...
            self.logger.error(
                "Canary deployment failed", model_name=config.model_name, error=str(e)
            )
            return False
    ...
    def rollback_deployment(self, model_name: str, version: str) -> bool:
        ...
        try:
            ...
            config = DeploymentConfig(
                ...
                health_check_config={"initial_delay": 30, "period": 10},
                rollback_config={"keep_old_deployment": True},
            )
            ...
            if success:
                # モデルステージの更新
                self.model_registry.transition_model_stage(
                    model_name, version, ModelStage.ARCHIVED
                )

                self.model_registry.transition_model_stage(
                    model_name, previous_version["version"], ModelStage.PRODUCTION
                )
            ...
            self.logger.error(
                "Rollback failed", model_name=model_name, version=version, error=str(e)
            )
            return False
