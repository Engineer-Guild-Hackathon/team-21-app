...
class MonitoringSystem:
    ...
    async def _collect_model_metrics(self) -> Dict[str, float]:
        ...
        try:
            # 現在のモデルバージョンの取得
            model_info = self.model_registry.get_model_history(
            ...
            self.logger.error(
                "Failed to collect model metrics",
                error=str(e)
            )
    ...
    async def _analyze_metrics(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(self.metrics_history) > 0:
                ...
                # 精度の低下
                if recent_metrics['prediction_accuracy'] < \
                ...
                # レイテンシーの上昇
                if recent_metrics['prediction_latency'] > \
            ...
            for component in ['model', 'api', 'database']:
                ...
                if memory_key in recent_metrics and \
                   recent_metrics[memory_key] > self.config.resource_thresholds['max_memory']:
                    alerts.append({
                ...
                if cpu_key in recent_metrics and \
                   recent_metrics[cpu_key] > self.config.resource_thresholds['max_cpu']:
                    alerts.append({
                        'type': 'high_cpu_usage',
                        'severity': AlertSeverity.WARNING,
            ...
            # ビジネスアラート
            if recent_metrics['user_satisfaction'] < \
    ...
    async def _generate_weekly_report(self) -> Dict[str, Any]:
        ...
        weekly_metrics = self.metrics_history[
            self.metrics_history['timestamp'] > \
            (datetime.now() - pd.Timedelta(weeks=1))
        ]
        
        # 日ごとの集計
        daily_stats = weekly_metrics.groupby(
            weekly_metrics['timestamp'].dt.date
        ).agg({
    ...
    async def _analyze_resource_trend(self,
        ...
        for component in ['model', 'api', 'database']:
            ...
            if memory_key in metrics.columns:
                resource_trends[f'{component}_memory'] = \
                    self._calculate_trend(metrics[memory_key])
            
            if cpu_key in metrics.columns:
                resource_trends[f'{component}_cpu'] = \
                    self._calculate_trend(metrics[cpu_key])
        
        return resource_trends
    
    async def _analyze_business_trend(self,
                                    metrics: pd.DataFrame) -> Dict[str, Any]:
        """ビジネストレンドの分析"""
        return {
    ...
    def _analyze_alert_trend(self, alerts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        alert_counts = pd.Series(
            [alert['timestamp'].date() for alert in alerts]
    ...
    async def _generate_visualizations(self,
                                     metrics: pd.DataFrame) -> Dict[str, str]:
        """可視化の生成"""
        visualizations = {}
    ...
    def _should_generate_daily_report(self) -> bool:
        ...
        now = datetime.now()
        should_generate = (
            now.date() > self._last_daily_report.date() and
            now.hour >= 0  # 0時以降
        )
    ...
    def _should_generate_weekly_report(self) -> bool:
        ...
        now = datetime.now()
        should_generate = (
            now.isocalendar()[1] > self._last_weekly_report.isocalendar()[1] and
            now.weekday() == 0 and  # 月曜日
            now.hour >= 0  # 0時以降
        )
