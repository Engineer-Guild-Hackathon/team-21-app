...
class ModelRegistry:
    ...
    def load_model(self, name: str, version: Optional[str] = None,
                  stage: Optional[ModelStage] = None) -> Tuple[nn.Module, ModelMetadata]:
        """モデルの読み込み"""
        try:
            # バージョンまたはステージの指定を確認
    ...
    def transition_model_stage(self, name: str, version: str,
    ...
    def _validate_stage_transition(self, current_stage: ModelStage,
    ...
    def import_model(self, import_path: str) -> Tuple[str, str]:
        ...
        try:
            ...
            model.load_state_dict(
                torch.load(import_dir / "model.pth")
            )
