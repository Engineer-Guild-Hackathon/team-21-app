...
from ..models.emotion_analyzer import EmotionAnalysisResult, EmotionCategory
from ..models.feedback_generator import (FeedbackContext, FeedbackGenerator,
                                         FeedbackStyle, FeedbackTemplate)
...
def test_feedback_style_determination(
    feedback_generator, mock_emotion_result, mock_learning_state, mock_task_history
):
    """フィードバックスタイルの決定テスト"""
...
def test_template_selection(feedback_generator):
    ...
    template = feedback_generator._select_template(
        EmotionCategory.MOTIVATED, FeedbackStyle.ENCOURAGING
    )
...
def test_context_info_extraction(
    feedback_generator, mock_emotion_result, mock_learning_state, mock_task_history
):
    """コンテキスト情報抽出のテスト"""
    context = FeedbackContext(
...
def test_next_goals_determination(
    feedback_generator, mock_emotion_result, mock_learning_state, mock_task_history
):
    """次の目標決定のテスト"""
    context = FeedbackContext(
...
def test_different_emotion_states(
    feedback_generator, mock_learning_state, mock_task_history
):
...
def test_different_feedback_styles(
    feedback_generator, mock_emotion_result, mock_learning_state, mock_task_history
):
    """異なるフィードバックスタイルのテスト"""
...
def test_empty_task_history(
    feedback_generator, mock_emotion_result, mock_learning_state
):
...
def test_extreme_learning_states(
    feedback_generator, mock_emotion_result, mock_task_history
):
