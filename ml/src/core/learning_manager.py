...
class LearningManager:
    ...
    def process_user_interaction(
        ...
        emotional_state = self.emotion_analyzer.get_emotional_state(
            text=text, facial_expression=facial_expression, behavior_data=behavior_data
        )

        # 強化学習による次のアクションの決定
        next_action = self.dqn_agent.get_action_for_emotional_state(
            emotional_state=emotional_state, learning_progress=self.learning_state
        )
    ...
    def update_learning_progress(self, quest_result: Dict[str, any], reward: float):
        """
        学習進捗を更新
        ...
        # レベルの更新
        if (
            self.learning_state["success_rate"] > 0.7
            and len(self.learning_state["completed_quests"]) > 5
        ):
