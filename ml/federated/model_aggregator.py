...
class ModelAggregator:
    ...
    def initialize_global_model(
        self, model_type: ModelType, initial_parameters: Dict[str, torch.Tensor]
    ):
        """グローバルモデルの初期化"""
        ...
        self.logger.info("Initialized global model", model_type=model_type.value)
    ...
    async def aggregate_updates(
        ...
        try:
            ...
            self.logger.error("Failed to aggregate models", error=str(e))
            raise
    ...
    def add_client_update(self, update: ModelUpdate):
        ...
        try:
            ...
            if not self._validate_update(update):
                self.logger.warning("Invalid client update", client_id=update.client_id)
                return
            ...
            self.logger.error(
                "Failed to add client update", client_id=update.client_id, error=str(e)
            )
            raise
    ...
    def _validate_update(self, update: ModelUpdate) -> bool:
        ...
        if update.model_type not in self.global_models:
            self.logger.error("Unknown model type", model_type=update.model_type.value)
            return False
        ...
        if set(update.parameters.keys()) != set(global_params.keys()):
            self.logger.error(
                "Parameter structure mismatch", client_id=update.client_id
            )
            return False
        ...
        if not required_metrics.issubset(update.metrics.keys()):
            self.logger.error("Missing required metrics", client_id=update.client_id)
            return False
    ...
    def _verify_updates(self, updates: List[ModelUpdate]) -> bool:
        ...
        for update in updates:
            ...
            for name, param in update.parameters.items():
                relative_change = torch.norm(param - global_params[name]) / torch.norm(
                    global_params[name]
                )
    ...
    def _compute_weights(self, updates: List[ModelUpdate]) -> Dict[str, float]:
        ...
        if self.config.weight_method == "uniform":
            ...
            total_performance = sum(
                1.0 / (update.metrics["loss"] + 1e-8) for update in updates
            )
            weights = {
                update.client_id: (1.0 / (update.metrics["loss"] + 1e-8))
                / total_performance
                for update in updates
            }
    ...
    def _aggregate_models(
        self, updates: List[ModelUpdate], weights: Dict[str, float]
        ...
        if self.config.aggregation_method == "fedavg":
            ...
            for name in updates[0].parameters.keys():
                weighted_sum = torch.zeros_like(
                    updates[0].parameters[name], device=self.device
                )
            ...
            for name in updates[0].parameters.keys():
                stacked_params = torch.stack(
                    [update.parameters[name].to(self.device) for update in updates]
                )
    ...
    def save_global_model(self, model_type: ModelType, path: str):
        ...
        try:
            ...
            self.logger.info(
                "Saved global model", model_type=model_type.value, path=str(save_path)
            )
    ...
    def load_global_model(self, path: str) -> ModelType:
        ...
        try:
            ...
            self.logger.info(
                "Loaded global model", model_type=model_type.value, path=path
            )

            return model_type

        except Exception as e:
            self.logger.error("Failed to load global model", path=path, error=str(e))
            raise
