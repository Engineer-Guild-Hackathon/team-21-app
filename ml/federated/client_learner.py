...
class ClientLearner:
    ...
    def _initialize_model(self) -> nn.Module:
        ...
        if self.config.model_type == ModelType.DQN:
            return DuelingDQN(state_dim=8, action_dim=60, hidden_dim=256).to(
                self.device
            )
    ...
    def train_local_model(self) -> Optional[ModelUpdate]:
        ...
        try:
            ...
            self.logger.info("Local training completed", metrics=metrics)

            return protected_update

        except Exception as e:
            self.logger.error("Failed to train local model", error=str(e))
            raise
    ...
    def _train_epochs(
        self, train_data: List[Dict[str, torch.Tensor]]
    ) -> Dict[str, float]:
        """エポック単位の学習"""
    ...
    def _create_model_update(self, metrics: Dict[str, float]) -> ModelUpdate:
        """モデル更新の作成"""
        parameters = {
            name: param.data.cpu() for name, param in self.model.named_parameters()
        }
    ...
    def update_local_model(self, global_parameters: Dict[str, torch.Tensor]):
        ...
        try:
            ...
            self.logger.error("Failed to apply global model update", error=str(e))
            raise
    ...
    def save_local_model(self, path: str):
        ...
        try:
            ...
            self.logger.error(
                "Failed to save local model", path=str(save_path), error=str(e)
            )
            raise
    ...
    def load_local_model(self, path: str):
        ...
        try:
            ...
            self.logger.error("Failed to load local model", path=path, error=str(e))
            raise
