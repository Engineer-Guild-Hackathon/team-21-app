...
class PrivacyMechanism:
    ...
    def _generate_encryption_key(self) -> bytes:
        ...
        if self.config.encryption_method == "fernet":
            ...
            raise ValueError(
                f"Unsupported encryption method: {self.config.encryption_method}"
            )
    ...
    def apply_differential_privacy(
        ...
        try:
            ...
            self.logger.error("Failed to apply differential privacy", error=str(e))
            raise
    ...
    def _calculate_privacy_cost(self, parameters: Dict[str, torch.Tensor]) -> float:
        ...
        privacy_loss = self._get_privacy_spent(
            orders, rdp, target_delta=self.config.delta
        )
    ...
    def _get_privacy_spent(
        self, orders: List[float], rdp: List[float], target_delta: float
    ) -> float:
        """プライバシー損失の計算"""
        # 最適なεの探索
        eps = float("inf")
        ...
        for alpha, curr_rdp in zip(orders, rdp):
            if curr_rdp < float("inf"):
                current_eps = curr_rdp + (np.log(1 / target_delta) + np.log(alpha)) / (
                    alpha - 1
                )
    ...
    def _update_privacy_budget(self, privacy_cost: float):
        ...
        self.privacy_accountant["remaining_budget"] = max(
            0.0, self.config.epsilon - self.privacy_accountant["spent_budget"]
        )
    ...
    def encrypt_parameters(self, parameters: Dict[str, torch.Tensor]) -> bytes:
        ...
        try:
            ...
            self.logger.error("Failed to encrypt parameters", error=str(e))
            raise
    ...
    def decrypt_parameters(self, encrypted_data: bytes) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # パラメータの復元
            decrypted_params = {
                name: torch.from_numpy(np.frombuffer(param_bytes, dtype=np.float32)).to(
                    self.device
                )
            ...
            self.logger.error("Failed to decrypt parameters", error=str(e))
            raise
    ...
    def _encrypt_rsa(self, data: Dict[str, bytes]) -> bytes:
        ...
        public_key = serialization.load_pem_private_key(
            self.encryption_key, password=None
        ).public_key()
