...
class FederatedOrchestrator:
    ...
    def __init__(
        ...
        self.best_metrics = {"loss": float("inf"), "accuracy": 0.0, "round": 0}
    ...
    async def start_server(self, host: str = "localhost", port: int = 5555):
        ...
        try:
            ...
            self.logger.info("Server started", host=host, port=port)
    ...
    async def _main_loop(self):
        ...
        while self.current_round < self.config.max_rounds:
            try:
                # ラウンドの開始
                self.logger.info(
                    "Starting training round", round=self.current_round + 1
                )
        ...
        try:
            ...
            for client_id in clients:
                await self._send_message(
                    client_id,
                    {"type": "start_training", "round": self.current_round + 1},
                )
            ...
            self.logger.error("Failed to execute training round", error=str(e))
            return False
    ...
    async def _send_message(self, client_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            await self.socket.send_multipart([client_id.encode(), serialized_message])

        except Exception as e:
            self.logger.error(
                "Failed to send message", client_id=client_id, error=str(e)
            )
            raise
    ...
    async def _receive_update(self, client_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to receive update", client_id=client_id, error=str(e)
            )
            return None
    ...
    async def _aggregate_updates(
        ...
        try:
            ...
            self.logger.error("Failed to aggregate updates", error=str(e))
            raise
    ...
    async def _distribute_model(
        self, model: Dict[str, torch.Tensor], clients: List[str]
    ):
        """モデルの配布"""
        try:
            # モデルの暗号化
            ...
            for client_id in clients:
                await self._send_message(
                    client_id, {"type": "model_update", "parameters": encrypted_model}
                )

        except Exception as e:
            self.logger.error("Failed to distribute model", error=str(e))
            raise
    ...
    async def _save_checkpoint(self):
        ...
        try:
            ...
            self.logger.info(
                "Saved checkpoint", round=self.current_round, path=str(save_path)
            )

        except Exception as e:
            self.logger.error(
                "Failed to save checkpoint", round=self.current_round, error=str(e)
            )
            raise
    ...
    async def handle_client_connection(self, client_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(
                "Failed to handle client connection", client_id=client_id, error=str(e)
            )
            raise
        ...
        while True:
            try:
                ...
                for client_id in disconnected_clients:
                    del self.active_clients[client_id]
                    self.logger.warning(
                        "Client disconnected due to timeout", client_id=client_id
                    )

                await asyncio.sleep(60)  # 1分ごとに確認

            except Exception as e:
                self.logger.error("Error in client monitoring", error=str(e))
