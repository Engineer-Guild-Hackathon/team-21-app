...
class DataTransformer:
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        return SparkSession.builder \
            .appName("NonCog-DataTransformer") \
    ...
    def transform_student_activities(self, df):
        ...
        # 重み付けUDF
        weight_udf = udf(
            lambda activity: float(activity_weights.get(activity, 0.0)),
            DoubleType()
        )
        
        # アクティビティスコアの計算
        activity_scores = df \
            .withColumn("activity_weight",
                       weight_udf(col("activity_type"))) \
            .withColumn("activity_score",
                       col("activity_weight") * when(
                           col("success").isNotNull() & col("success"),
                           1.0
                       ).otherwise(0.5))
        ...
        transformed_activities = activity_scores \
            .withColumn("rolling_score",
                       sum("activity_score").over(window_spec)) \
            .withColumn("activity_count",
                       count("activity_type").over(window_spec)) \
            .withColumn("success_rate",
                       avg(when(col("success"), 1.0).otherwise(0.0))
                       .over(window_spec)) \
            .withColumn("activity_variety",
                       size(collect_set("activity_type").over(window_spec)))
    ...
    def transform_learning_metrics(self, df):
        ...
        transformed_metrics = normalized_metrics \
            .withColumn(
                "learning_efficiency",
                col("normalized_accuracy") * col("normalized_completion_rate") /
                greatest(col("normalized_time_spent"), lit(0.1))
            ) \
            .withColumn(
                "rolling_efficiency",
                avg("learning_efficiency").over(window_spec)
            ) \
            .withColumn(
                "improvement_rate",
    ...
    def transform_emotional_states(self, df):
        ...
        # 感情カテゴリのエンコーディング
        emotion_categories = [
            "motivated", "frustrated", "confused",
            "satisfied", "anxious", "confident", "bored"
        ]
        ...
        for emotion in emotion_categories:
            encoded_emotions = encoded_emotions \
                .withColumn(
                    f"is_{emotion}",
                    when(col("emotion") == emotion, 1).otherwise(0)
                )
        ...
        for emotion in emotion_categories:
            transformed_emotions = transformed_emotions \
                .withColumn(
        ...
        transformed_emotions = transformed_emotions \
            .withColumn(
                "emotional_stability",
                1.0 - stddev_pop(
                    array([col(f"{e}_frequency") for e in emotion_categories])
                )
            ) \
            .withColumn(
                "dominant_emotion",
                array_max(
                    array([
                        struct(col(f"{e}_frequency").alias("freq"),
                              lit(e).alias("emotion"))
    ...
    def create_integrated_features(self, activities_df, metrics_df, emotions_df):
        ...
        # アクティビティ特徴量
        activity_features = activities_df \
            .select(
                "student_id",
                "timestamp",
                "rolling_score",
        ...
        # 感情特徴量
        emotion_features = emotions_df \
            .select(
                "student_id",
                "timestamp",
                "emotional_stability",
                "dominant_emotion",
                *[f"{e}_frequency" for e in [
                    "motivated", "frustrated", "confused",
                    "satisfied", "anxious", "confident", "bored"
                ]]
            )
        ...
        # 特徴量の結合
        joined_features = activity_features \
            .join(
                metrics_features,
        ...
        # 欠損値の補完
        filled_features = joined_features \
            .na.fill({
                "rolling_score": 0,
                "activity_count": 0,
                "success_rate": 0.5,
        ...
        # 複合特徴量の計算
        integrated_features = filled_features \
            .withColumn(
                "engagement_score",
            ...
            .withColumn(
                ...
                (col("emotional_stability") * 0.5 +
                 when(col("dominant_emotion").isin(
                     ["motivated", "confident", "satisfied"]
                 ), 1.0).otherwise(0.0) * 0.5)
            )
    ...
    def aggregate_student_metrics(self, integrated_features):
        ...
        # 集計の実行
        aggregated_metrics = integrated_features \
            .withColumn(
                "daily_engagement",
                avg("engagement_score").over(daily_window)
            ) \
            .withColumn(
                "daily_learning",
                avg("learning_potential").over(daily_window)
            ) \
            .withColumn(
                "daily_emotional",
                avg("emotional_balance").over(daily_window)
            ) \
            .withColumn(
                "weekly_engagement",
                avg("engagement_score").over(weekly_window)
            ) \
            .withColumn(
                "weekly_learning",
                avg("learning_potential").over(weekly_window)
            ) \
            .withColumn(
                "weekly_emotional",
                avg("emotional_balance").over(weekly_window)
            ) \
            .withColumn(
                "engagement_trend",
                col("daily_engagement") - col("weekly_engagement")
            ) \
            .withColumn(
                "learning_trend",
                col("daily_learning") - col("weekly_learning")
            ) \
            .withColumn(
                "emotional_trend",
                col("daily_emotional") - col("weekly_emotional")
            )
