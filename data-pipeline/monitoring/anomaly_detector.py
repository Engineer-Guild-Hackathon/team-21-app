...
ANOMALY_COUNTER = Counter(
    "noncog_anomalies_total", "Total number of detected anomalies", ["type", "severity"]
)

SYSTEM_HEALTH = Gauge(
    "noncog_system_health", "Overall system health score", ["component"]
)
...
class AnomalyDetector:
    ...
    def _create_spark_session(self) -> SparkSession:
        """Sparkセッションの作成"""
        return (
            SparkSession.builder.appName("NonCog-AnomalyDetector")
        ...
        with PROCESSING_LATENCY.labels("statistical").time():
            ...
            for metric in metrics:
                ...
                zscore_anomalies = zscore_anomalies.withColumn(
                    f"{metric}_zscore", abs((col(metric) - mean_val) / stddev_val)
                ).withColumn(
    ...
    def detect_temporal_anomalies(self, df, time_col: str, metrics: List[str]):
        ...
        with PROCESSING_LATENCY.labels("temporal").time():
            # 時間窓の定義
            window_spec = Window.orderBy(time_col).rangeBetween(
                -timedelta(hours=1).total_seconds(), 0
            )
            ...
            for metric in metrics:
                # 移動平均と標準偏差の計算
                temporal_anomalies = (
                    temporal_anomalies.withColumn(
                        f"{metric}_ma", avg(col(metric)).over(window_spec)
                    )
                    .withColumn(
                        f"{metric}_stddev", stddev(col(metric)).over(window_spec)
                    )
                    .withColumn(
                        f"{metric}_change_rate",
                        abs(
                            (col(metric) - col(f"{metric}_ma"))
                            / greatest(col(f"{metric}_ma"), lit(0.1))
                        ),
                    )
                    .withColumn(
                        f"{metric}_temporal_anomaly",
                        col(f"{metric}_change_rate") > self.thresholds["change_rate"],
                    )
                )
            ...
            for i, metric1 in enumerate(metrics):
                for j, metric2 in enumerate(metrics[i + 1 :], i + 1):
                    if abs(correlation_values[i][j]) > self.thresholds["correlation"]:
                        # 期待される相関からの逸脱を検出
                        correlation_anomalies = correlation_anomalies.withColumn(
    ...
    def _calculate_distance_udf(self, features_col: str, center_col: str):
        """クラスタ中心からの距離計算UDF"""
        return udf(
            lambda features, center: float(
                np.sqrt(sum((f - c) ** 2 for f, c in zip(features, center)))
            ),
            DoubleType(),
    ...
    def detect_system_anomalies(self, metrics_df):
        ...
        with PROCESSING_LATENCY.labels("system").time():
            # システムメトリクスの集計
            system_metrics = metrics_df.groupBy("timestamp").agg(
            ...
            # 総合スコアの計算
            system_health = system_health.withColumn(
                "overall_health",
                (
                    col("cpu_score")
                    + col("memory_score")
    ...
    def generate_anomaly_report(self, anomalies_df):
        """異常検知レポートの生成"""
        # 異常の集計
        anomaly_summary = anomalies_df.select(
            [
        ...
        for col_name in [c for c in anomalies_df.columns if "anomaly" in c]:
            ...
            if count > 10:
                critical_anomalies.append({"type": col_name, "count": count})
            elif count > 0:
                warning_anomalies.append({"type": col_name, "count": count})

        # メトリクスの更新
        for anomaly in critical_anomalies:
            ANOMALY_COUNTER.labels(type=anomaly["type"], severity="critical").inc(
                anomaly["count"]
            )

        for anomaly in warning_anomalies:
            ANOMALY_COUNTER.labels(type=anomaly["type"], severity="warning").inc(
                anomaly["count"]
            )
    ...
    def process_streaming_anomalies(self, input_stream_df, output_path: str):
        ...
        def process_batch(batch_df, batch_id):
            try:
                ...
                metrics = [
                    f.name
                    for f in batch_df.schema.fields
                    if isinstance(f.dataType, (IntegerType, DoubleType, FloatType))
                ]

                # 各種異常検知の実行
                anomalies = batch_df
                anomalies = self.detect_statistical_anomalies(anomalies, metrics)
                anomalies = self.detect_temporal_anomalies(
                    anomalies, "timestamp", metrics
                )
                anomalies = self.detect_correlation_anomalies(anomalies, metrics)
                anomalies = self.detect_pattern_anomalies(anomalies, metrics)
                ...
                anomalies.write.mode("overwrite").parquet(
                    f"{output_path}/anomalies/batch_{batch_id}"
                )

                system_health.write.mode("overwrite").parquet(
                    f"{output_path}/system_health/batch_{batch_id}"
                )

                data_quality.write.mode("overwrite").parquet(
                    f"{output_path}/data_quality/batch_{batch_id}"
                )
                ...
                self.logger.error(
                    "Failed to process anomalies", batch_id=batch_id, error=str(e)
                )
                raise
