...
MESSAGE_SIZE = Histogram(
    "kafka_producer_message_size_bytes", "Size of produced messages in bytes", ["topic"]
)

PRODUCE_LATENCY = Histogram(
    "kafka_producer_latency_seconds", "Latency of message production", ["topic"]
)
...
class KafkaProducer:
    ...
    async def produce_student_activity(
    ...
    async def produce_learning_metrics(
        self, student_id: str, metrics: Dict[str, float]
    ):
        """学習メトリクスの送信"""
        message = {
            "student_id": student_id,
    ...
    async def produce_emotional_state(
        self, student_id: str, emotional_state: Dict[str, Any]
    ):
        """感情状態の送信"""
        message = {
            "student_id": student_id,
    ...
    async def produce_feedback_event(
        self, student_id: str, feedback_data: Dict[str, Any]
    ):
        """フィードバックイベントの送信"""
        message = {
            "student_id": student_id,
        ...
        try:
            ...
            with PRODUCE_LATENCY.labels(topic=topic).time():
                ...
                self.producer.produce(
                    topic=topic, value=value, callback=self.delivery_callback
                )

                # バッファのフラッシュ
                self.producer.poll(0)

            self.logger.info("Message produced", topic=topic, message_size=len(value))
...
class BatchKafkaProducer(KafkaProducer):
    ...
    def __init__(
        self, config: Dict[str, Any], batch_size: int = 100, flush_interval: float = 1.0
    ):
    ...
    async def _produce_message(self, topic: str, message: Dict[str, Any]):
        ...
        if (
            self.message_count >= self.batch_size
            or (now - self.last_flush_time).total_seconds() >= self.flush_interval
        ):
            await self.flush()
            self.message_count = 0
...
class RetryKafkaProducer(KafkaProducer):
    ...
    def __init__(
        self, config: Dict[str, Any], max_retries: int = 3, retry_interval: float = 1.0
    ):
        super().__init__(config)
