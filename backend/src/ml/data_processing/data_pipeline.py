...
class DataPipeline:
    ...
    def process_user_session(
        ...
        emotion_data = self._process_emotion_data(
            user_id,
            session_data.get("emotions", [])
        )
        
        # 2. 行動データの処理
        behavior_data = self._process_behavior_data(
            user_id,
            session_data.get("behaviors", [])
        )
        ...
        # 4. 非認知能力スコアの計算
        noncog_scores = self._calculate_noncog_scores(
            emotion_data,
            behavior_data
        )
        ...
        for emotion in self.emotion_weights.keys():
            ...
            emotion_trends[emotion] = {
                ...
                "trend": self._calculate_trend(
                    emotion_df[mask]["intensity"].tolist()
                )
            }
    ...
    def _analyze_correlations(
        ...
        for emotion in self.emotion_weights.keys():
            for behavior in self.behavior_weights.keys():
                emotion_times = emotion_df[
                    emotion_df["emotion_type"] == emotion
                ]["timestamp"]
                
                behavior_times = behavior_df[
                    behavior_df["action_type"] == behavior
                ]["timestamp"]
    ...
    def _calculate_persistence_score(
        ...
        problem_solving_rate = behavior_patterns.get("problem_solving", {}).get("count", 0)
