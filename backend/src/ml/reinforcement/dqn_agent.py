...
class DQNAgent:
    ...
    def _calculate_reward(
        ...
        # 行動に基づく報酬
        action_rewards = {
            "challenge": 0.5 if new_state["emotion"] in ["joy", "concentration"] else -0.3,
    ...
    def train(self, batch: List[Dict[str, Any]]) -> float:
        ...
        # データの準備
        states = torch.FloatTensor([self._encode_state(d["state"]) for d in minibatch]).to(self.device)
        actions = torch.LongTensor([self.actions.index(d["action"]) for d in minibatch]).to(self.device)
        rewards = torch.FloatTensor([d["reward"] for d in minibatch]).to(self.device)
        next_states = torch.FloatTensor([self._encode_state(d["next_state"]) for d in minibatch]).to(self.device)
