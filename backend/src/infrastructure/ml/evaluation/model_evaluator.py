...
class ModelEvaluator:
    ...
    def evaluate_emotion_analyzer(
        self, model: EmotionAnalyzer, test_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """感情分析モデルの評価"""
        y_true = []
        y_pred = []
    ...
    def evaluate_dialogue_system(
        self, model: DialogueSystem, test_conversations: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """対話システムの評価"""
        metrics = {
            "response_appropriateness": [],
        ...
        for conv in test_conversations:
            ...
            emotion_consistency = self._calculate_emotion_consistency(
                response["emotion"], context.get("emotion", "neutral")
            )
    ...
    def evaluate_dqn_agent(
        self, agent: DQNAgent, test_episodes: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """強化学習エージェントの評価"""
        episode_rewards = []
    ...
    def _calculate_response_appropriateness(
        self, generated_response: str, expected_response: str
    ) -> float:
        """応答の適切性を計算（0-1のスコア）"""
    ...
    def _calculate_emotion_consistency(
        self, predicted_emotion: str, context_emotion: str
    ) -> float:
        """感情の一貫性を計算（0-1のスコア）"""
        # 感情の類似度を定義
        emotion_similarity = {
    ...
    def _plot_dqn_metrics(
        self, episode_rewards: List[float], action_counts: Dict[str, int]
    ):
        """DQNエージェントの評価指標を可視化"""
        # 報酬の推移
...
def evaluate_all_models():
    ...
    try:
        ...
        emotion_results = evaluator.evaluate_emotion_analyzer(
            emotion_analyzer, test_data
        )

        print("対話システムの評価中...")
        dialogue_results = evaluator.evaluate_dialogue_system(
            dialogue_system, test_conversations
        )

        print("強化学習エージェントの評価中...")
        dqn_results = evaluator.evaluate_dqn_agent(dqn_agent, test_episodes)
