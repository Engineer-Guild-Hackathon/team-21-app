...
class EmotionDataCollector:
    ...
    def collect_from_database(self, db: Session) -> Dict[str, Any]:
        ...
        # データの整形
        collected_data = {"emotions": [], "behaviors": [], "correlations": []}
        ...
        for record in behavior_records:
            behavior_data = {
                ...
                "timestamp": record.created_at.isoformat(),
                "success_rate": (
                    record.success_rate if hasattr(record, "success_rate") else None
                ),
            }
        ...
        if not df_emotions.empty and not df_behaviors.empty:
            ...
            for _, emotion in df_emotions.iterrows():
                related_behaviors = df_behaviors[
                    ...
                    & (
                        abs(df_behaviors["timestamp"] - emotion["timestamp"])
                ...
                for _, behavior in related_behaviors.iterrows():
                    correlation = {
                        ...
                        "time_diff": (
                            behavior["timestamp"] - emotion["timestamp"]
                        ).total_seconds(),
                    }
    ...
    def save_collected_data(self, data: Dict[str, Any]):
        ...
        emotions_df.to_csv(
            os.path.join(self.data_dir, f"emotions_{timestamp}.csv"), index=False
        )
        behaviors_df.to_csv(
            os.path.join(self.data_dir, f"behaviors_{timestamp}.csv"), index=False
        )
        correlations_df.to_csv(
            os.path.join(self.data_dir, f"correlations_{timestamp}.csv"), index=False
        )
    ...
    def check_data_quality(self, data: Dict[str, Any]) -> List[str]:
        ...
        for emotion in self.collection_config["emotions"]:
            ...
            if count < self.collection_config["min_samples_per_emotion"]:
                issues.append(
                    f"感情 '{emotion}' のサンプル数が不足しています（{count}/{self.collection_config['min_samples_per_emotion']}）"
                )
        ...
        # タイムスタンプの妥当性チェック
        df_emotions["timestamp"] = pd.to_datetime(df_emotions["timestamp"])
        future_records = df_emotions[df_emotions["timestamp"] > datetime.now()]
        if not future_records.empty:
            issues.append(
                f"未来の日時を持つレコードが {len(future_records)} 件あります"
            )
