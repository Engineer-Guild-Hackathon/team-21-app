...
from ..domain.models.behavior import BehaviorRecord, ActionType
...
async def get_behavior_stats(
    ...
    # 基本統計の計算
    total_problems = query.filter(BehaviorRecord.action_type == ActionType.PROBLEM_START).count()
    total_attempts = query.filter(BehaviorRecord.action_type == ActionType.PROBLEM_SUBMIT).count()
    ...
    # その他の統計
    hint_requests = query.filter(BehaviorRecord.action_type == ActionType.HINT_REQUEST).count()
    give_ups = query.filter(BehaviorRecord.action_type == ActionType.GIVE_UP).count()
    collaborations = query.filter(BehaviorRecord.action_type == ActionType.COLLABORATION).count()
    reflections = query.filter(BehaviorRecord.action_type == ActionType.REFLECTION).count()
    
    return {
        "total_problems_attempted": total_problems,
        "average_attempts_per_problem": total_attempts / total_problems if total_problems > 0 else 0,
